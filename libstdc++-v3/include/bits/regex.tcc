...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  template<typename _BiIter, typename _Alloc,
	...
	__it.matched = false;
      ...
      __ret = _Regex_scope<_Comparable_iter<_BiIter>>::template __run<__policy>(_Comparable_iter<_BiIter>(std::move(__s)), _Comparable_iter<_BiIter>(std::move(__e)), __res, *__re._M_automaton, __flags, __search_mode);
      if (__ret)
	{
	  for (auto& __it : __res)
	    if (!__it.matched)
